...
sumoCmd = [sumoBinary, "-c", "D:/sumo code/SUMO_ACCIDENT/SUMO_ACCIDENT/Configuration.sumo.cfg"]
traci.start(sumoCmd)
...
def log_event(receiver_vehicle, sender_vehicle, message_type):
    """Logs the event into the data list with vehicle position."""
    event_data = {
        "receiver_vehicle": receiver_vehicle,
        "sender_vehicle": sender_vehicle,
        "message_type": message_type,
        "timestamp": traci.simulation.getTime(),
        "position": traci.vehicle.getPosition(receiver_vehicle)  # Add position
    }
...
for step in range(130):  # Simulate for 100 steps.
    ...
    for vehicle_id in vehicle_ids:
        if random.random() < 0.01:
            ...
            # Capture the traffic data when the event occurs
            stopped_vehicle_data = {
                "id": vehicle_id,
            ...
            traffic_data_list.append(stopped_vehicle_data)  # Store the stopped vehicle separately
